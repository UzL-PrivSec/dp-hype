...
class FedSum(Strategy):
    ...
    def configure_fit(
        ...
        num_hps = len(self.hyperparams)

        if num_hps > len(clients):
            raise ValueError(
                f"Number of hyperparameter configurations ({num_hps}) cannot be greater than the number of sampled clients ({len(clients)})."
            )

        fitins_list = []
        for idx, client in enumerate(clients):
            config_copy = config.copy()
            for subset_idx, subset in enumerate(self.subset_indices):
                if idx in subset:
                    config_copy["subset_idx"] = subset_idx
                    break
            fitins = FitIns(parameters, config_copy)
            fitins_list.append(fitins)

        return list(zip(clients, fitins_list))
