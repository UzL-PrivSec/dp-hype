...
class FedSum(Strategy):
    ...
    def configure_fit(
        ...
        print("##############################")
        print(self.hyperparams[server_round - 1]["lr"], self.hyperparams[server_round - 1]["lr_decay"])

        sample_size, min_num_clients = self.num_fit_clients(client_manager.num_available())
        clients = client_manager.sample(num_clients=sample_size, min_num_clients=min_num_clients)
        return [(client, FitIns(parameters, config)) for idx, client in enumerate(clients)]
    ...
    def aggregate_evaluate(
        ...
        total_loss = sum(evaluate_res.loss for _, evaluate_res in results)  # No weighting here either (sum)
        metrics_aggregated = {}
